...
void rotate(const float* input, float* output, const int width, const int height, char* degree)
{
    ...
    err = clSetKernelArg(kernel, 4, sizeof(cl_float), &x0);
    CHECK_ERROR(err);

    err = clSetKernelArg(kernel, 5, sizeof(cl_float), &y0);
    CHECK_ERROR(err);
    
    err = clSetKernelArg(kernel, 6, sizeof(cl_float), &sin_theta);
    CHECK_ERROR(err);

    err = clSetKernelArg(kernel, 7, sizeof(cl_float), &cos_theta);
    CHECK_ERROR(err);
